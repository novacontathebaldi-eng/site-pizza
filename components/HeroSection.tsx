...
export const HeroSection: React.FC<HeroSectionProps> = ({ siteContent }) => {
    ...
    const titleParts = siteContent?.headerTitle.split('Santa Sensação');
    ...
    return (
        <section id="inicio" className="bg-brand-green-700 text-text-on-dark min-h-[calc(100vh-80px)] flex items-center justify-center pb-20 px-4 relative overflow-hidden">
            <div 
                ...
                style={{
                    backgroundImage: `url(${siteContent?.heroBgUrl})`,
            ...
            <div className="container mx-auto text-center z-10">
                <div className="inline-block bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20 mb-6">
                    <p className="font-semibold text-sm flex items-center gap-2"><i className="fas fa-award text-brand-gold-600"></i>{siteContent?.heroBadge}</p>
                </div>
                <h1 className="text-4xl md:text-6xl font-extrabold mb-4 leading-tight">
                    {titleParts && titleParts.length > 1 ? (
                        <>
                            {titleParts[0]}
                            <span className="text-brand-gold-600">Santa Sensação</span>
                            {titleParts[1]}
                        </>
                    ) : (
