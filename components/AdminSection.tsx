...
export const AdminSection: React.FC<AdminSectionProps> = (props) => {
    ...
    const SortableProductItem: React.FC<{
        ...
        return (
            <div ref={setNodeRef} style={style} className={`flex flex-col lg:flex-row items-start lg:items-center gap-4 p-3 rounded-lg transition-colors ${!product.active || !isCategoryActive ? 'bg-gray-100 opacity-60' : 'bg-white'}`}>
                <div className="flex items-center w-full lg:w-auto">
                    <button type="button" {...attributes} {...listeners} className="cursor-grab p-2 text-gray-400 hover:text-gray-600">
                        <i className="fas fa-grip-vertical"></i>
                    </button>
                    <img src={product.imageUrl} alt={product.name} className="w-16 h-16 rounded-md object-cover flex-shrink-0 ml-2" />
                    <div className="flex-grow ml-4">
                        <p className="font-bold">{product.name}</p>
                        <p className="text-sm text-gray-500">{Object.entries(product.prices).map(([size, price]) => `${size}: R$${price.toFixed(2)}`).join(' / ')}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 w-full lg:w-auto justify-end mt-2 lg:mt-0">
                    <select 
    ...
    const SortablePromotionItem: React.FC<{ promotion: Promotion }> = ({ promotion }) => {
        ...
        return (
             <div ref={setNodeRef} style={style} className={`flex flex-col lg:flex-row items-start lg:items-center gap-4 p-3 rounded-lg ${!promotion.active ? 'bg-gray-100 opacity-60' : 'bg-white'}`}>
                <div className="flex items-center w-full lg:w-auto">
                    <button type="button" {...attributes} {...listeners} className="cursor-grab p-2 text-gray-400 hover:text-gray-600">
                        <i className="fas fa-grip-vertical"></i>
                    </button>
                    <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center border overflow-hidden flex-shrink-0 ml-2">
                ...
                <div className="container mx-auto px-4">
                    <div className="bg-white p-4 sm:p-8 rounded-2xl shadow-lg border border-gray-200">
                        <div className="flex justify-between items-center mb-6 pb-4 border-b">
                            <h2 className="text-2xl sm:text-3xl font-bold">Painel Administrativo</h2>
