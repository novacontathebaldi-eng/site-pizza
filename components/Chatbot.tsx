...
export const Chatbot: React.FC<ChatbotProps> = ({ isOpen, onClose, messages, onSendMessage, isSending }) => {
    ...
    return (
        <div 
            aria-hidden={!isOpen}
            className={`fixed bottom-4 right-4 w-full max-w-sm h-[70vh] max-h-[600px] bg-white rounded-2xl shadow-2xl z-50 flex flex-col transform transition-all duration-300 ease-in-out ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0 pointer-events-none'}`}
        >
