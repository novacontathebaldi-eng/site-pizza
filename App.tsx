...
const App: React.FC = () => {
    ...
    const handleProductStatusChange = useCallback(async (productId: string, active: boolean) => { try { await supabaseService.updateProductStatus(productId, active); addToast(`Produto ${active ? 'ativado' : 'desativado'}.`, 'success'); if (!active) { const changedProduct = products.find(p => p.id === productId); if (changedProduct) { const { categoryId } = changedProduct; const hasOtherActiveProducts = products.some(p => p.categoryId === categoryId && p.id !== productId && p.isActive && !p.isDeleted); if (!hasOtherActiveProducts) { await supabaseService.updateCategoryStatus(categoryId, false); const category = categories.find(c => c.id === categoryId); addToast(`Categoria "${category?.name || 'desconhecida'}" desativada pois todos os seus produtos estão inativos.`, 'success'); } } } } catch (error) { console.error("Failed to update product status:", error); addToast("Erro ao atualizar status.", 'error'); } }, [addToast, products, categories]);
