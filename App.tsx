...
const App: React.FC = () => {
    ...
    const handleSaveProduct = useCallback(async (product: Product) => { try { const { id, ...dataToSave } = product; if (id) { const originalProduct = products.find(p => p.id === id); if (originalProduct && originalProduct.categoryId !== product.categoryId) { const oldCategoryId = originalProduct.categoryId; const remainingProductsInOldCategory = products.filter(p => p.id !== id && p.categoryId === oldCategoryId && !p.isDeleted); const shouldDeactivateOldCategory = remainingProductsInOldCategory.length === 0 || remainingProductsInOldCategory.every(p => !p.isActive); if (shouldDeactivateOldCategory) { await supabaseService.updateCategoryStatus(oldCategoryId, false); const category = categories.find(c => c.id === oldCategoryId); addToast(`Categoria "${category?.name || 'Anterior'}" desativada por estar vazia ou ter apenas produtos inativos.`, 'success'); } } await supabaseService.updateProduct(id, dataToSave); } else { await supabaseService.addProduct({ ...dataToSave, sortOrder: products.length, stockStatus: 'available' }); } addToast(id ? "Produto atualizado!" : "Produto adicionado!", 'success'); } catch (error) { console.error("Failed to save product:", error); addToast("Erro ao salvar produto.", 'error'); } }, [products, categories, addToast]);
    const handleDeleteProduct = useCallback(async (productId: string) => { try { const productToDelete = products.find(p => p.id === productId); if (productToDelete) { const { categoryId } = productToDelete; const remainingProductsInCategory = products.filter(p => !p.isDeleted && p.id !== productId && p.categoryId === categoryId); const shouldDeactivate = remainingProductsInCategory.length === 0 || remainingProductsInCategory.every(p => !p.isActive); if (shouldDeactivate) { await supabaseService.updateCategoryStatus(categoryId, false); const category = categories.find(c => c.id === categoryId); addToast(`Categoria "${category?.name || 'desconhecida'}" desativada por estar vazia ou ter apenas produtos inativos.`, 'success'); } } await supabaseService.deleteProduct(productId); addToast("Produto movido para a lixeira!", 'success'); } catch (error) { console.error("Failed to delete product:", error); addToast("Erro ao mover para a lixeira.", 'error'); } }, [products, categories, addToast]);
    const handleProductStatusChange = useCallback(async (productId: string, active: boolean) => { try { await supabaseService.updateProductStatus(productId, active); setProducts(prev => prev.map(p => p.id === productId ? { ...p, isActive: active } : p)); addToast(`Produto ${active ? 'ativado' : 'desativado'}.`, 'success'); if (!active) { const changedProduct = products.find(p => p.id === productId); if (changedProduct) { const { categoryId } = changedProduct; const hasOtherActiveProducts = products.some(p => p.categoryId === categoryId && p.id !== productId && p.isActive && !p.isDeleted); if (!hasOtherActiveProducts) { await supabaseService.updateCategoryStatus(categoryId, false); setCategories(prev => prev.map(c => c.id === categoryId ? { ...c, isActive: false } : c)); const category = categories.find(c => c.id === categoryId); addToast(`Categoria "${category?.name || 'desconhecida'}" desativada pois todos os seus produtos estão inativos.`, 'success'); } } } } catch (error) { console.error("Failed to update product status:", error); setProducts(prev => [...prev]); /* trigger re-render on failure */ addToast("Erro ao atualizar status.", 'error'); } }, [addToast, products, categories]);
    const handleProductStockStatusChange = useCallback(async (productId: string, stockStatus: 'available' | 'out_of_stock') => { try { await supabaseService.updateProductStockStatus(productId, stockStatus); setProducts(prev => prev.map(p => p.id === productId ? { ...p, stockStatus } : p)); addToast(`Estoque atualizado.`, 'success'); } catch (error) { console.error("Failed to update product stock status:", error); setProducts(prev => [...prev]); /* trigger re-render on failure */ addToast("Erro ao atualizar estoque.", 'error'); } }, [addToast]);
    ...
    const handleSaveCategory = useCallback(async (category: Category) => { try { const { id, ...dataToSave } = category; if (id) await supabaseService.updateCategory(id, dataToSave); else await supabaseService.addCategory({ ...dataToSave, sortOrder: categories.length }); addToast(id ? "Categoria atualizada!" : "Categoria adicionada!", 'success'); } catch (error) { console.error("Failed to save category:", error); addToast("Erro ao salvar categoria.", 'error'); } }, [categories.length, addToast]);
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-grow">
                ...
                {isLoading ? <div className="text-center py-20"><i className="fas fa-spinner fa-spin text-5xl text-accent"></i><p className="mt-4 text-xl font-semibold text-gray-600">Carregando cardápio...</p></div> : !error && <MenuSection categories={categories} products={products.filter(p => !p.isDeleted)} onAddToCart={handleAddToCart} isStoreOnline={isStoreOnline} activeCategoryId={activeMenuCategory} setActiveCategoryId={setActiveMenuCategory} cartItemCount={cartTotalItems} onCartClick={() => setIsCartOpen(true)} cartItems={cart} onSelectHalfAndHalf={handleSelectHalfAndHalf} menuViewMode={menuViewMode} onMenuViewChange={handleMenuViewChange} onShowProductDetails={handleShowProductDetails} />}
